      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {usuarioActual ? (
              <>
                {/* Gamification Level & XP Badge (Compact) */}
                <button
                  onClick={() => setTabActual('clasificacion')}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-amber-500/10 border border-amber-400/30 hover:bg-amber-500/20 transition-all text-xs font-black text-amber-800"
                  title={`Nivel ${usuarioActual.nivel} • ${usuarioActual.xp} XP. Ver Tablas de Clasificación`}
                >
                  <span>⚡ Nv. {usuarioActual.nivel}</span>
                  <span className="text-[10px] font-mono font-bold text-amber-600">({usuarioActual.xp} XP)</span>
                </button>
